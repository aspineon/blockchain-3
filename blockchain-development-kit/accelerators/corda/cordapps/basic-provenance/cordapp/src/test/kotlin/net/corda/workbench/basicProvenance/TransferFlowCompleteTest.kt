...
class TransferFlowCompleteTests {
    ...
    fun flowHappyPath() {
        ...
        val transferCompleteFlow = TransferCompleteFlow(item.linearId, otherParty)
        val future3 = c.startFlow(transferCompleteFlow)
        mockNetwork.runNetwork()
        ...
        assert(ptx3.tx.outputs.single().data is ItemState)
        val command = ptx3.tx.commands.single()
        assert(command.value is ItemContract.TransferComplete)
